...
const DivMarginLeft = ({ index = 0, level = 0, children }) => (
  <div
    key={`${level}_${index}`}
    style={{
...
const JsonForest = ({ json, level = 0, show = false }) => {
  const [expanded, setExpanded] = useState(
    new Array(Object.keys(json).length).fill(false)
  );

  const onExpand = useCallback(
    index => () => {
      let newShow = [...expanded];
  ...
  return (
    <div>
      {Object.keys(json).map((key, index) => {
        ...
        return (
          <DivMarginLeft index={index} level={level}>
            <div onClick={onExpand(index)}>
              <span>{`${key}:  ${json[key]}             `}</span>
              {isExpandable && (
                <span>
            ...
            {isExpandable && expanded[index] && (
              <JsonForest json={json[key]} level={level + 1} show={expanded} />
            )}
          </DivMarginLeft>
        );
      })}
